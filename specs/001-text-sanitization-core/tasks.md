...
- [X] T001 Create the Electron + TypeScript + React project structure per plan.md's Project Structure (src/domain, src/main, src/preload, src/renderer subfolders; tests/unit, tests/contract, tests/integration)
- [X] T002 Initialize package.json with pinned TypeScript 5.9.3 and an exact-pinned Electron **43.1.0** (no `^`/`~` range) — re-verified at implementation time (2026-07-19) against the Electron 43.1.1 / Node 24.18.0 planning baseline per plan.md Technical Context; corrected down to 43.1.0 because 43.1.1 was published within the project's 7-day dependency-freshness window at implementation time, confirmed by running it that it bundles the identical Node 24.18.0 the baseline assumed (a dependency-baseline correction, not a product requirement change — see plan.md Technical Context "Implementation baseline" note)
- [X] T003 [P] Add and pin React, Monaco Editor, better-sqlite3, and Zod as production dependencies with `--save-exact` (research.md #1, #3, #4)
- [X] T004 [P] Add and pin Vitest and Playwright (`@playwright/test` with Electron support) as dev dependencies with `--save-exact` (research.md #7)
- [X] T005 [P] Configure tsconfig.json with strict mode and separate compile targets for src/main, src/preload, and src/renderer
- [X] T006 [P] Configure ESLint and Prettier for the project
- [X] T007 [P] Configure vitest.config.ts for domain-layer unit tests and IPC contract tests
- [X] T008 [P] Configure playwright.config.ts for Electron end-to-end tests
- [X] T009 [P] Add .gitignore entries for `*.sqlite`, `*.sqlite-wal`, `*.sqlite-shm`, `*.sqlite-journal`, and any local dev-data directory (research.md #10 "Preventing accidental Git inclusion")
- [X] T010 Implement a temporary per-test/per-dev-run userData directory helper so tests and `npm run dev` never touch the real userData path (research.md #10)
- [X] T011 [P] Implement the GitHub Actions CI workflow (`.github/workflows/ci.yml`) with a **macOS + Windows job matrix** that runs, in order: a **specification-validation** step (re-running `.specify/scripts/bash/check-prerequisites.sh` plus a script asserting tasks.md has sequential, non-duplicate task IDs and that every `[Story]`-labeled task's cited FR/SC IDs exist in spec.md), a **Prettier format-check** step, an **ESLint** step, and a **`tsc --noEmit`** type-checking step across src/main, src/preload, and src/renderer — gating every pull request to main (constitution Principle VI required quality gates: specification validation, formatting, linting, type checking)
- [X] T012 Extend the CI workflow (`.github/workflows/ci.yml`) with the **Vitest unit/contract suites** and the **Playwright e2e suite**, run as blocking steps on both matrix legs (constitution Principle VI "automated test suites" gate) (depends on T011)
- [X] T013 Extend the CI workflow (`.github/workflows/ci.yml`) with a **dependency-vulnerability audit** step (`npm audit --audit-level=high`, or equivalent) governed by an explicit failure policy — the job MUST fail on any actionable (fixable) high/critical advisory and MUST NOT silently ignore or globally suppress audit failures; a vulnerability with no available fix may only be waived via a committed, reviewed allowlist entry naming the advisory ID, the justification, and an expiry date, after which the waiver must be re-justified or the job fails again — plus an **application build-verification** step that runs the Electron packaging/build command on both the macOS and Windows matrix legs and fails the job on any build error (constitution Principle VI "dependency audit" and "build verification for every supported platform" gates) (depends on T011)
...
- [X] T041 [P] Integration test for single-instance enforcement: a second launch attempt never opens SQLite, never unwraps a key, never starts a worker, and the first instance's window is focused instead, written and confirmed to fail before app-lifecycle.ts exists, in tests/integration/single-instance.spec.ts (constitution Principle VI; research.md #14)
- [X] T042 Implement app-lifecycle.ts: acquire `app.requestSingleInstanceLock()` as the first startup action other than the narrow, production-inert userData-path resolution research.md #14 documents (packaged-Windows `%LOCALAPPDATA%` override, dev/test isolated-directory override); call `app.quit()` immediately if it returns `false`; otherwise open registry.sqlite, unwrap the registry DEK, run startup deletion reconciliation (T040), wire the `workspace:create`/`workspace:open` IPC handlers (T036) against the now-open registry connection, and only then create the main `BrowserWindow`; register a `second-instance` handler that focuses/restores the existing window, making T041 pass, in src/main/app-lifecycle.ts (research.md #14) (depends on T028, T036, T040)

### Worker-thread infrastructure

- [X] T043 [P] Unit test for worker crash recovery: simulate a worker `'error'`/`'exit'` event and assert in-flight requests fail cleanly and a freshly recreated worker is reseeded with the current dictionary snapshot, written and confirmed to fail before worker-manager.ts exists, in tests/unit/main/worker-manager.test.ts (constitution Principle VI; research.md #11) — **Revision note (T044/T045 ordering-defect remediation, this date)**: extended, in the same file, with additional unit tests for two other worker-manager.ts responsibilities named in T045's own text that the original three crash-recovery assertions didn't cover — reseed-only-on-version-change (including reseeding a recreated worker with the most-recently-seen version, not its original one) and `requestId`-based supersession bookkeeping (dropping a superseded older response as `SUPERSEDED`, scoped per (workspace, direction), never across directions or workspaces) — plus one cross-workspace-isolation test (constitution Principle II) confirming a crash/recreation in one workspace's worker never affects another's, and one requestId-collision-across-directions test added after an independent post-implementation review found that `requestId` is only unique *within* one direction (research.md #11: "scoped per (workspace, direction) pair"), so a sanitize request and a restore request can legitimately carry the same numeric `requestId` — this test (and the corresponding `pending`-map keying fix in worker-manager.ts, keyed by `(direction, requestId)` rather than `requestId` alone, with `translation-worker.ts`'s response messages now echoing `direction` so the response can be routed) was written and confirmed to fail first, exactly like the rest. All are written and confirmed to fail before worker-manager.ts exists, exactly like the original three.
- [X] T044 Implement translation-worker.ts: the `worker_threads` entry point handling the message-transport/protocol boundary — a `"seed"` message stores the current dictionary snapshot, a `"translate"` message is dispatched by direction — in src/main/workers/translation-worker.ts, with its own unit tests (`handleTranslationWorkerMessage` exercised directly, no real worker thread needed) written and confirmed to fail first, in tests/unit/main/translation-worker.test.ts (research.md #11; constitution Principle VI) — **Revision note (T044/T045 ordering-defect remediation, this date)**: redefined, ID kept stable. The original wording ("wiring domain/sanitizer and domain/restorer to parentPort messaging") created a genuine Foundational-phase ordering defect — it required domain/sanitizer (T079, US1) and domain/restorer (T090, US2) to exist before Foundational (which every user story depends on, tasks.md lines 55/369) could be considered complete. This module now never imports domain/sanitizer or domain/restorer; for a `"translate"` message it returns the real, honest `{requestId, ok: false, error: "HANDLER_NOT_CONFIGURED"}` response (not a stub simulating translation success/failure) until T079/T090 each replace their own branch with a real call. Building/reusing the matching structures (the Aho-Corasick automaton, the placeholder-token scanner) from the snapshot happens inside sanitize.ts/restore.ts (T078/T079, T089/T090) once wired in here, not in this transport layer — no responsibility is dropped, only relocated to where it already belonged. (depends on T028)
- [X] T045 Implement per-workspace worker orchestration: worker creation/termination as an opaque-worker lifecycle primitive, `postMessage` seeding only on snapshot-version change (including reseeding a recreated worker with the most-recently-seen version), `requestId`-based supersession bookkeeping scoped per (workspace, direction), and crash detection (`'error'`/`'exit'`) with automatic worker recreation and reseeding, making T043 (as extended) pass, in src/main/persistence/worker-manager.ts (research.md #11) (depends on T044) — **Revision note (T044/T045 ordering-defect remediation, this date)**: implemented against the extended T043 test suite, independent of T044's actual message-handling content (T043's tests inject a fake worker via dependency injection and never resolve translation-worker.ts's real path or content — confirmed by both independent reviewers before implementation). Three sub-responsibilities from the original wording are deliberately NOT implemented here, since nothing exists yet to trigger or consume them: (a) triggering worker creation specifically on workspace-open/first-request, and termination on workspace-close/app-exit — that call-site wiring belongs to whichever task actually opens/closes a workspace against a live worker (T081/T092); (b) an incrementing dictionary version counter tied to actual Term/Placeholder/Prefix/PendingDecision mutations — no such mutation exists yet (US3, T101-T108); (c) the push notification (IPC event to the renderer) whenever that counter changes — already explicitly assigned by this task's own prior text to T114, US3's reactive-recompute consumer, which still owns it. This module supplies the snapshot-version-comparison mechanism (a) and (b)/(c) will drive once they exist, exactly as T081/T092 already supply the decrypted-snapshot-building step this module's `ensureWorker` consumes.
...
- [ ] T079 [US1] Implement sanitizer orchestration: lookup-before-allocate (FR-SANITIZE-001), the identical placeholder for every occurrence (FR-SANITIZE-002), exact preservation of non-matched text (FR-SANITIZE-004), and leaving NEVER-policy terms and undecided ambiguous terms unchanged (FR-SANITIZE-006, FR-DECISION-002), in src/domain/sanitizer/sanitize.ts; also replace translation-worker.ts's `"sanitize"` `HANDLER_NOT_CONFIGURED` branch (T044) with a real call into this module, updating tests/unit/main/translation-worker.test.ts's corresponding assertion to check real sanitized output instead (depends on T044, T078)
...
## T044/T045 ordering-defect remediation (2026-07-23)

**Defect**: T044's original wording ("the `worker_threads` entry point wiring domain/sanitizer and domain/restorer to `parentPort` messaging") required domain/sanitizer (T079, US1) and domain/restorer (T090, US2) to exist before it could be implemented — but T045 (worker-manager.ts) depends on T044, and both sit in Foundational (Phase 2), which this document's own Phase Dependencies section (and its Foundational-phase Checkpoint) states must fully complete *before any user story begins*. T041/T042 were already implemented; T043's test (tests/unit/main/worker-manager.test.ts) was written and confirmed to genuinely fail (`Cannot find module '.../worker-manager'`) because T045 was blocked behind T044, which was itself blocked behind US1/US2. A prior session flagged this, implemented only T041-T043, and stopped for correction rather than guessing.

**Resolution** (independently reviewed by specification-reviewer and test-reviewer before implementation, no unresolved normative conflict found, no spec.md-level product-behavior conflict — this is a task-sequencing/documentation-accuracy correction only):

- **T044, ID kept stable, redefined**: scope narrowed to only the transport/protocol boundary (message handling + snapshot storage), which needs neither domain/sanitizer nor domain/restorer. Its own test-first requirement was folded into its own task text (tests/unit/main/translation-worker.test.ts, written and confirmed to fail before src/main/workers/translation-worker.ts existed), rather than spawning a new task ID for it, since inserting a new numbered task in natural position would require renumbering every task from T046 onward (`scripts/validate-tasks.mjs` requires a strictly sequential T001..N sequence with no gaps) — an unacceptably disruptive way to add test coverage for one small, already-tightly-scoped module.
- **T045 implemented now**, confirmed independent of T044's real content: its own test (T043) already used dependency injection (a fake worker via `WorkerManagerDeps.createWorker`), never resolving translation-worker.ts's real path or content.
- **T043 extended** (same file, additional `it()` blocks, same ID) with test-first coverage for two more T045 responsibilities the original three crash-recovery assertions didn't reach: reseed-only-on-version-change, and `requestId`-based supersession bookkeeping — plus one cross-workspace-isolation test. All written and confirmed failing before worker-manager.ts existed, alongside the original three.
